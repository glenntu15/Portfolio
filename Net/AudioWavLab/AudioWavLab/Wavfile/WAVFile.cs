...
namespace EricOulashin
{
    ...
    class WAVFile
    {
        ...
        // 2014-07-27 GAT added properties
        public int SampleRate { get { return mSampleRateHz; } set { mSampleRateHz = SampleRate; } }
        public int NumChanels     { get { return (int)mNumChannels;    } set { mNumChannels    = (byte)NumChanels; } }
        public int BytesPerSample { get { return (int)mBytesPerSample; } set { mBytesPerSample = (short)BytesPerSample; } }
        ...
        public String Open(WAVFileMode pMode)
        {
            ...
            try
            {
                ...
                char c1 = (char)buffer[0];
                char c2 = (char)buffer[1];
                char c3 = (char)buffer[2];
                char c4 = (char)buffer[3];
                buffer.CopyTo(mWAVHeader, 0);
                // Read the file size (4 bytes)  chunk size?
                mFileStream.Read(buffer, 0, 4);
                
                //mFileSizeBytes = BitConverter.ToInt32(buffer, 0);
                // Read the RIFF type
                mFileStream.Read(buffer, 0, 4);
                char dbc1 = (char)buffer[0];
                char dbc2 = (char)buffer[1];
                char dbc3 = (char)buffer[2];
                char dbc4 = (char)buffer[3];
                buffer.CopyTo(mRIFFType, 0);

                // Format chunk (24 bytes total)
                // "fmt " (ASCII characters)
                mFileStream.Read(buffer, 0, 4);
                char c5 = (char)buffer[0];
                char c6 = (char)buffer[1];
                char c7 = (char)buffer[2];
                char c8 = (char)buffer[3];
        ...
        /// byte per channel if the file contains 8-bit audio or 2 bytes per
        /// channel if the file contains  16-bit audio. 
        /// The total number of bytes is:  mBytesPerSample
        /// The return value will be null if no
        ...
        public byte[] GetNextSample_ByteArray()
        {
            ...
            try
            {
                ...
                audioSample = new byte[mBytesPerSample];
                mFileStream.Read(audioSample, 0, mBytesPerSample);
        ...
        public float[] GetNextSample_FloatArray()
        {
            float[] audioSample = null;
            byte[] temp;
            ...
            if (mBytesPerSample % 4 != 0)
                throw new WAVFileReadException("Read float array attempted when not floats: " + WAVFileModeStr(mFileMode), "WAVFile.GetNextSample_FloatArray()");
            try
            {
                temp = new byte[BytesPerSample];
                audioSample = new float[mBytesPerSample/4];
                mFileStream.Read(temp, 0, mBytesPerSample);
                audioSample[0] = BitConverter.ToSingle(temp, 0);
                audioSample[1] = BitConverter.ToSingle(temp, 4);
                --mNumSamplesRemaining;
            }
            catch (Exception exc)
            {
